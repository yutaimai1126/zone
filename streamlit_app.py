...
def zone_estimation():
    ...
    while cap.isOpened() and not stop_button_pressed:
        ...
        st.session_state.focus_score = (st.session_state.nomal_time +  st.session_state.zone_time*1.1 - st.session_state.lazy_time) * 100 / st.session_state.study_time
    ...
    st.text(f'勉強時間 : {round(st.session_state.study_time,0)}秒')
    st.text(f'集中した時間 : {round(st.session_state.zone_time,0)}秒')

    if st.session_state.study_time > 0:
        labels = ['ZONE', 'LAZY', 'NOMAL']
        sizes = [st.session_state.zone_time, st.session_state.lazy_time, st.session_state.nomal_time]
        colors = ['#ff9999','#66b3ff','#99ff99']
        fig, ax = plt.subplots()
        # ax.title('Focus result')
